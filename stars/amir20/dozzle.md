...
Most recent release. Recommended for most users.

`v10.6.15`

An exact release. Pin this for reproducible deployments.

`v10.6`

Latest patch within that minor version. Picks up bug fixes only.

`v10`

Latest release within that major version. Picks up new features too.

`alpine`

Same as `latest`, but on an Alpine base instead of `scratch`.

`v10.6.15-alpine`, `v10.6-alpine`, `v10-alpine`

Alpine variants of the version tags above.

`master`

Built from the `master` branch on every push. Unreleased and unstable.

`pr-1234`, `pr-1234-alpine`

Built from pull request #1234, for testing a fix before it ships.
