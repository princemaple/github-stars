...
-   Fil-C's capability-based approach achieves a similar level of safety to hardware capabilities like CHERI, except that it runs on stock hardware (X86\_64 or ARM64).
...
-   In case of the classic musl-based distribution (`filc-0.685-linux-x86_64.tar.xz` or `filc-0.685-linux-aarch64.tar.xz`), this sets up Fil-C to run in the current directory.
    
-   In case of the /opt/fil glibc-based distribution (`optfil-0.685-linux-x86_64.tar.xz`), this sets up Fil-C in `/opt/fil`.
