...
> 💡 **My current best practice:** Use **EdgeEver** to quickly capture ideas and reminders as a raw "material library." When content needs structured organization, use **MCP** to let AI automatically organize and sync it into **Obsidian**, **Notion Database**, or **Feishu Bitable**.
...
After the first deployment, see Cloudflare Workers Builds for automatic updates. The same deployment flow is used by official instances and forks.
...
Please refer to the Cloudflare Manual Deployment Guide for first-time manual installation, Cloudflare resource setup, and emergency recovery. After the first deployment, connect Workers Builds; future updates arrive through GitHub **Sync fork** or pushes to `main`.
