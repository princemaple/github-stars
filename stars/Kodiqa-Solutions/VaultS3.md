...
Reed-Solomon encode/reconstruct and the background healer have fault-injection tests (lose disks → reconstruct → heal).
...
-   **CLI tool**: Standalone `vaults3-cli` binary for bucket, object, user, and replication management without AWS CLI, plus `vaults3-cli info` for server version and storage capacity (used / free / total), `vaults3-cli cluster` for day-2 cluster operations (status, join, leave, drain/undrain a member, rebalance, decommission — see docs/SCALING.md), and `vaults3-cli storage reclaim` to free data files that no metadata refers to any more
...
-   **Storage classes**: STANDARD and REDUCED\_REDUNDANCY storage class support
...
`POST/PUT/DELETE /{bucket}/{key}?uploads&uploadId`

Done

UploadPartCopy

`PUT /{bucket}/{key}?partNumber&uploadId` + `x-amz-copy-source`
...
`PUT/GET/DELETE /{bucket}/{key}?tagging`
...
`PUT/GET/DELETE /{bucket}?lifecycle`

Done

Website Hosting

`PUT/GET/DELETE /{bucket}?website`

Done

Bucket CORS

`PUT/GET/DELETE /{bucket}?cors`
...
`PUT/GET/DELETE /{bucket}?encryption`

Done

Public Access Block

`PUT/GET/DELETE /{bucket}?publicAccessBlock`

Done

Bucket Logging

`PUT/GET /{bucket}?logging`
...
Dashboard API `/api/v1/iam/*`

Done

STS Temporary Credentials

`POST /api/v1/sts/session-token`
...
`PUT /api/v1/iam/users/{name}/ip-restrictions`

Done

Bucket Notifications

`PUT/GET/DELETE /{bucket}?notification`
...
`GET/POST/DELETE /api/v1/versions/tags`
...
`GET/PUT/DELETE /api/v1/lambda/triggers/{bucket}`

Done

Lambda Status

`GET /api/v1/lambda/status`

Done

Bucket Versioning (Dashboard)

`GET/PUT /api/v1/buckets/{name}/versioning`

Done

Bucket Lifecycle (Dashboard)

`GET/PUT/DELETE /api/v1/buckets/{name}/lifecycle`

Done

Bucket CORS (Dashboard)

`GET/PUT/DELETE /api/v1/buckets/{name}/cors`

Done

Bulk Delete (Dashboard)

`POST /api/v1/buckets/{name}/bulk-delete`

Done

Bulk Download Zip

`GET /api/v1/buckets/{name}/download-zip?keys=...`

Done

Version List (Dashboard)

`GET /api/v1/versions?bucket=X&key=Y`
...
`PUT/GET/DELETE /{bucket}?replication`
...
`POST /{bucket}` (multipart/form-data)

Done

Get Object (Part)

`GET /{bucket}/{key}?partNumber=N`
...
logging:
  enabled: false
  file\_path: "./access.log"
  level: "info"  # debug, info, warn, error

lifecycle:
  scan\_interval\_secs: 3600

security:
  ip\_allowlist: \[\]     # global CIDR allow list, empty = allow all
  ip\_blocklist: \[\]     # global CIDR deny list
  audit\_retention\_days: 90
  sts\_max\_duration\_secs: 43200  # max STS token duration (12 hours)
...
cluster:
  ...
  placement:
    ...
    virtual\_nodes: 128
  detector:
    probe\_interval\_secs: 5
    suspect\_after: 3
    down\_after: 6
  rebalance:
    max\_bandwidth\_mbps: 50
    batch\_size: 100
...
erasure:
  ...
  data\_shards: 4
  parity\_shards: 2
  block\_size: 4194304      # 4MB, objects smaller than this bypass EC
  data\_dirs: \[\]            # multiple disk paths for shard distribution
  heal\_interval: 300       # seconds between heal scans

# Replication
replication:
  enabled: false
  mode: "push"             # "push" (one-way) or "active-active" (bidirectional)
  site\_id: "site-1"        # unique site ID for active-active mode
  conflict\_strategy: "last-writer-wins"  # "last-writer-wins", "largest-object", "site-preference"
  preferred\_site: ""       # for site-preference strategy
  peers: \[\]
  scan\_interval\_secs: 10
  max\_retries: 5
  batch\_size: 100
...
server:
  tls:
    enabled: true
    cert\_file: "/path/to/cert.pem"
    key\_file: "/path/to/key.pem"
...
`VAULTS3_METADATA_DIR`

BoltDB metadata directory
...
-   One file per object means a filesystem with plenty of inodes (XFS handles this well). For workloads with millions of tiny objects, enable the experimental small-file packing mode to pack them into large volume files and cut per-file overhead.
-   On Kubernetes, a CSI driver like **DirectPV** is a good fit, it formats disks with XFS and presents them as mounted PVCs, which is exactly what VaultS3 wants.

### Kubernetes

Deploy with the bundled **Helm chart** or a single **plain-manifest** quickstart (both under `deploy/`):
...
The Helm chart can also **auto-form a Raft cluster** (Beta) with `--set cluster.enabled=true --set replicaCount=3`: pod-0 bootstraps as leader and the others auto-join, with self-healing on pod restart. Metadata writes replicate across nodes via Raft consensus (a write to any node is forwarded to the leader), so the cluster stays consistent. Clustering is functional but newer than single-node + erasure coding, which remains the recommendation for maximum production durability.

For backup/restore workflows, the chart also supports a single-node **Deployment** mode (`controller.kind=Deployment`) and **existing PVCs** (`persistence.data.existingClaim`), so you can mount a claim restored from Velero, k8up, or a CSI snapshot, see the chart reference.
...
s3 \= boto3.client('s3', endpoint\_url\='http://localhost:9000',
    aws\_access\_key\_id\='vaults3-admin',
    aws\_secret\_access\_key\='vaults3-secret-change-me')

\# Enable versioning
s3.put\_bucket\_versioning(Bucket\='my-bucket',
    VersioningConfiguration\={'Status': 'Enabled'})

\# Upload creates a new version each time
s3.put\_object(Bucket\='my-bucket', Key\='file.txt', Body\=b'v1')
s3.put\_object(Bucket\='my-bucket', Key\='file.txt', Body\=b'v2')

\# Get specific version
s3.get\_object(Bucket\='my-bucket', Key\='file.txt', VersionId\='...')

\# Delete creates a delete marker (versions preserved)
s3.delete\_object(Bucket\='my-bucket', Key\='file.txt')

\# Permanently delete a specific version
s3.delete\_object(Bucket\='my-bucket', Key\='file.txt', VersionId\='...')
...
s3.put\_object\_legal\_hold(Bucket\='my-bucket', Key\='file.txt', VersionId\='...',
    LegalHold\={'Status': 'ON'})

\# Retention — prevents deletion until date
s3.put\_object\_retention(Bucket\='my-bucket', Key\='file.txt', VersionId\='...',
...
s3.put\_bucket\_lifecycle\_configuration(Bucket\='my-bucket',
...
For workloads with huge numbers of tiny objects, packing stores small objects as independent zstd frames inside large append-only **volume** files (with byte-offset locations in BoltDB) instead of one file per object, avoiding per-file overhead. Objects larger than `max_object_size` are stored as individual files as usual.

packing:
  enabled: true
  max\_object\_size: 1048576       # objects this size (bytes) or smaller are packed
  volume\_max\_size: 1073741824    # roll to a new volume past this size
  compact\_interval\_hours: 24     # background dead-space reclamation; 0 = disabled
  compact\_min\_dead\_ratio: 0.5    # compact a volume once half of it is dead space

Deleted/overwritten objects leave dead space in volumes. It is reclaimed by background compaction (or on demand via `POST /api/v1/compact`). Packing is **experimental** and does not yet compose with encryption or erasure coding (it is skipped, with a warning, if either is enabled).
...
s3.put\_bucket\_website(Bucket\='my-site',
...
requests.post(f"{API}/iam/policies", headers\=headers,
    json\={"name": "MyBucketReadOnly", "document": custom\_policy})
...
s3.put\_bucket\_cors(Bucket\='my-bucket', CORSConfiguration\={
    'CORSRules': \[{
...
s3 \= boto3.client("s3", endpoint\_url\="http://localhost:9000",
    aws\_access\_key\_id\=creds\["accessKey"\],
    aws\_secret\_access\_key\=creds\["secretKey"\])
...
security:
  ip\_allowlist: \["10.0.0.0/8", "192.168.0.0/16"\]  # empty = allow all
  ip\_blocklist: \["10.0.0.99/32"\]  # deny always wins
...
notif\_xml \= b"""<?xml version="1.0" encoding="UTF-8"?>
...
req \= AWSRequest(method\="PUT", url\=url, data\=notif\_xml,
    headers\={"Content-Type": "application/xml"})
SigV4Auth(creds, "s3", "us-east-1").add\_auth(req)
requests.put(url, headers\=dict(req.headers), data\=notif\_xml)
...
notifications:
  max\_workers: 4       # concurrent webhook delivery goroutines
  queue\_size: 256      # buffered event queue size
  timeout\_secs: 10     # webhook HTTP timeout
  max\_retries: 3       # retry attempts for failed webhooks
  kafka:
    enabled: true
  ...
  redis:
    ...
    list\_key: ""                # set for LPUSH queue mode
  amqp:
    enabled: true
    url: "amqp://guest:guest@localhost:5672/"
    exchange: "vaults3-events"
    routing\_key: "s3.events"
  postgres:
    enabled: true
...
replication:
  ...
  peers:
    - name: "dc2"
      ...
      access\_key: "peer-admin"
      secret\_key: "peer-secret"
  scan\_interval\_secs: 30   # queue processing interval
  max\_retries: 5           # retry before dead-letter
  batch\_size: 100          # events per scan cycle
...
export VAULTS3\_ENDPOINT=http://localhost:9000
export VAULTS3\_ACCESS\_KEY=vaults3-admin
export VAULTS3\_SECRET\_KEY=vaults3-secret-change-me
...
requests.put(url, data\=image\_data, headers\={"Content-Type": "image/jpeg"})
...
scanner:
  enabled: true
  webhook\_url: "http://localhost:3310/scan"
  timeout\_secs: 30
  quarantine\_bucket: "vaults3-quarantine"
  fail\_closed: false          # false=fail-open (keep file), true=quarantine on error
  max\_scan\_size\_bytes: 104857600  # 100MB
  workers: 2
...
tiering:
  enabled: true
  cold\_data\_dir: "./cold\_data"
  migrate\_after\_days: 30
  scan\_interval\_secs: 3600
...
backup:
  ...
  schedule\_cron: "0 2 \* \* \*"   # daily at 2am
  retention\_days: 30
...
curl "http://localhost:9000/api/v1/versions/diff?bucket=my-bucket&key=file.txt&v1=VERSION\_A&v2=VERSION\_B" \\
  -H "Authorization: Bearer <token>"
...
curl -X POST http://localhost:9000/api/v1/versions/tags \\
  -H "Authorization: Bearer <token>" \\
  -H "Content-Type: application/json" \\
  -d '{"bucket":"my-bucket","key":"file.txt","versionId":"VERSION\_ID","tag":"v1.0"}'
...
curl -X POST http://localhost:9000/api/v1/versions/rollback \\
  -H "Authorization: Bearer <token>" \\
  -H "Content-Type: application/json" \\
  -d '{"bucket":"my-bucket","key":"file.txt","versionId":"VERSION\_ID"}'
...
Input formats: CSV (with/without headers, custom delimiters), JSON Lines, JSON Document (array), Parquet (columnar format via parquet-go). Compressed input: GZIP and BZIP2 compressed CSV/JSON files are transparently decompressed before query execution. Output formats: JSON (one object per line) or CSV.
...
s3.put\_object(Bucket\='my-bucket', Key\='file.txt', Body\=b'protected')
...
rate\_limit:
  enabled: true
  requests\_per\_sec: 100   # per client IP
  burst\_size: 200
  per\_key\_rps: 50         # per access key
  per\_key\_burst: 100
...
VaultS3 can check GitHub Releases once a day and show a **dashboard banner** when a newer version is out. Updates only ever replace the binary or image, your object data, metadata, and config are never touched.

**Docker (recommended): Watchtower** watches for a new image and recreates the container. Your data volumes are preserved:
...
**Binary / systemd:** enable the built-in updater in `vaults3.yaml`. With `apply: true` it downloads the new release for your platform, **verifies its SHA-256 checksum**, swaps the binary, and restarts into the new version (checked daily. Never auto-crosses a major version):
...
-   Core S3 CRUD operations
-   S3 Signature V4 authentication
-   Presigned URLs
-   Content-Type detection and storage
-   Range requests (partial GET)
-   Copy object (same/cross-bucket)
-   Batch delete
-   Multipart upload (full lifecycle)
-   Multiple access keys
-   Object tagging
-   AES-256-GCM encryption at rest
-   Bucket policies (public-read, custom)
-   Quota management (per-bucket)
-   Virtual-hosted style URLs
-   Prometheus-compatible metrics
-   Web dashboard with built-in UI (login, bucket browser, file management, access keys, activity log, stats, dark/light theme, responsive)
-   Health check endpoints (/health, /ready)
-   Graceful shutdown (SIGTERM/SIGINT with configurable timeout)
-   TLS support (HTTPS with cert/key)
-   Docker image (multi-stage build with health check)
-   Object versioning (per-bucket, version IDs, delete markers, version-specific operations)
-   Object locking / WORM (legal hold, retention with GOVERNANCE/COMPLIANCE modes)
-   Lifecycle rules (per-bucket expiration with background worker)
-   Gzip compression (transparent compress/decompress)
-   Access logging (structured JSON lines)
-   Static website hosting (index/error documents, no-auth serving)
-   IAM users, groups & policies (fine-grained access control, policy evaluation engine, built-in policies)
-   CORS per bucket (S3-compatible, OPTIONS preflight)
-   STS temporary credentials (short-lived keys, auto-cleanup, configurable max duration)
-   Audit trail (persistent log, filtering by user/bucket/time, auto-pruning)
...
-   Data tiering (hot/cold storage, automatic migration based on access patterns, transparent reads, manual migration API)
-   Backup scheduler (full/incremental backups to local targets, cron scheduling, backup history, trigger API)
-   Git-like versioning (visual diff with LCS, version tagging with labels, one-click rollback)
-   FUSE mount (mount buckets as local filesystem, read/write, lazy loading via range requests)
-   Rate limiting (token bucket per IP and per access key, 429 responses, auto-cleanup)
-   UploadPartCopy (copy byte ranges from existing objects as multipart parts)
-   S3 Select (SQL queries on CSV and JSON objects, SELECT/WHERE/LIMIT/LIKE/AND/OR)
-   Multi-backend notifications (Kafka, NATS, Redis pub/sub and queue backends)
-   Bucket default retention (auto-apply GOVERNANCE/COMPLIANCE retention to new objects)
-   Per-bucket Prometheus metrics (request counts, bytes in/out, errors by bucket label)
-   OIDC/JWT SSO (dashboard login via Google/Keycloak/Auth0, RS256 JWKS verification, domain filtering, auto-create users, role mapping)
-   Lambda compute triggers (webhook functions on S3 events, event/key filtering, optional body inclusion, output storage, worker pool)
-   FUSE read cache (LRU block cache, metadata TTL cache, kernel attribute caching, SigV4 key caching)
-   RAM optimization (slim search index with LRU cap, batched last-access writes, GOMEMLIMIT support)
-   Dashboard advanced pages (IAM users/groups/policies, audit trail, search, notifications, replication, lambda triggers, backups, 7 new pages with full CRUD)
-   GetBucketLocation, Bucket Tagging, Bucket/Object ACL, ListMultipartUploads, ListParts (6 new S3 operations for AWS CLI/SDK compatibility)
-   Structured logging with slog (key-value pairs, configurable log level)
-   Request ID middleware (X-Request-Id header on every response)
-   Panic recovery middleware (stack trace logging, graceful 500 responses)
-   Request latency histogram (Prometheus-compatible `vaults3_request_duration_seconds_bucket`)
-   Go unit tests for 14 packages (metadata, storage, IAM, ratelimit, search, config, middleware, server, lifecycle, S3 handler, API, versioning, notify), 133 tests
-   Makefile targets: `make test-coverage`, `make lint`
-   Dashboard polish: sortable table columns on all list pages (files, buckets, keys, audit, search)
-   Dashboard polish: client-side pagination (50 items/page) on file browser and audit trail
-   Dashboard polish: file preview panel (images, text files, JSON) with metadata sidebar
-   Dashboard polish: bucket config UI (versioning toggle, lifecycle rule editor, CORS config editor)
-   Dashboard API: versioning, lifecycle, and CORS endpoints for bucket settings management
-   SVG dashboard charts: BarChart (per-bucket sizes), DonutChart (request method distribution), Sparkline (activity)
-   GitHub Actions CI: build, test, lint, coverage on push/PR
-   pprof debug endpoint: `/debug/pprof/*` behind `debug: true` config flag
-   Extended stats API with request metrics (total requests, errors, bytes in/out, requests by method)
-   Dashboard bulk operations: multi-select with bulk delete and bulk zip download
-   Dashboard drag-and-drop folder upload (recursive directory reading via webkitGetAsEntry)
-   Dashboard auto-refresh toggle on stats page (30s interval, localStorage persistence)
-   Dashboard copy-to-clipboard buttons for S3 URIs, object keys, and access keys
-   Dashboard API: bulk-delete and download-zip endpoints
-   Dashboard home page with summary cards, activity sparkline, system stats, quick actions
-   Settings page: read-only server configuration viewer (`GET /api/v1/settings`, secrets stripped)
-   Keyboard shortcuts: `/` go to search, `?` shortcut help overlay, `Esc` close modal/blur
-   Breadcrumb navigation in file browser with home icon, chevron separators, clickable path segments
-   Security headers middleware (CSP, X-Frame-Options, X-Content-Type-Options, HSTS, Referrer-Policy)
-   CORS origin validation (same-origin + localhost, replaces wildcard)
-   Dashboard API rate limiting (429 Too Many Requests)
-   Input validation (DNS-compatible bucket names, object key constraints)
-   Object versioning dashboard UI (version list panel, diff viewer, rollback, version tagging, bucket versioning indicator)
-   Version list API (`GET /api/v1/versions?bucket=X&key=Y`)
-   Toast notification system (success/error/info toasts, bottom-right stack, auto-dismiss 5s)
-   Sortable table columns on all dashboard pages (notifications, backups, lambda triggers, replication queue)
-   Enhanced bucket creation wizard (optional versioning toggle + auto-expire lifecycle rule)
-   API tests (auth login/failure, JWT required, bucket CRUD, CORS preflight, input validation)
-   Versioning tests (LCS diff algorithm, text type detection, metadata diff, tag store CRUD)
-   Notify tests (dispatcher lifecycle, backend publish, webhook delivery, event/key filtering, retry)
-   Python integration test (security headers, CORS, auth, validation, bucket CRUD, versioning, settings)
-   User metadata (x-amz-meta-\*) on PUT/GET/HEAD
-   Conditional GET/PUT (If-Modified-Since, If-None-Match, If-Match → 304/412)
-   Content-MD5 validation on PUT
-   Metadata-only copy (x-amz-metadata-directive: REPLACE)
-   Conditional copy (x-amz-copy-source-if-\* headers)
-   Response header overrides (?response-content-type, etc.)
-   Inline tagging and retention on PUT
-   Canned ACL headers (x-amz-acl, x-amz-grant-\*)
-   S3 Checksum API (CRC32, CRC32C, SHA1, SHA256)
-   Parts count header on HEAD (x-amz-mp-parts-count)
-   ListObjectsV1 (marker-based pagination)
-   ListBuckets with prefix filter
-   Versioning suspend support
-   GetObject by part number (?partNumber=N)
-   Advanced lifecycle rules (NoncurrentVersionExpiration, AbortIncompleteMultipartUpload, MaxNoncurrentVersions, ExpiredObjectDeleteMarker cleanup, object size filter)
-   IAM policy conditions (StringEquals, StringLike, IpAddress, DateLessThan)
...
-   Access key description & status
-   Governance bypass (x-amz-bypass-governance-retention)
-   Per-bucket replication rules with prefix/tag filters
-   S3 Replication Config API (PUT/GET/DELETE /{bucket}?replication)
-   Real-time event-driven replication
-   Existing object replication
-   Delete marker replication
-   Site replication (IAM + bucket config sync)
-   KMS integration (HashiCorp Vault + local key provider)
-   Remote tiering (S3-compatible cold tier)
-   RestoreObject API (POST /{bucket}/{key}?restore)
-   Storage classes (STANDARD, REDUCED\_REDUNDANCY)
-   Compression exclusions (skip already-compressed file types)
-   Real-time event streaming (SSE at /api/v1/events)
-   Real-time log streaming (SSE at /api/v1/logs)
-   Request tracing (SSE at /api/v1/trace)
-   Health diagnostics (/api/v1/diagnostics)
-   Manual heal API (POST /api/v1/heal)
-   Speedtest (POST /api/v1/speedtest)
-   Batch operations processor (bulk delete/copy)
-   PROXY protocol v1 support
-   Auto-TLS (Let's Encrypt + self-signed fallback)
-   Inter-node network separation
-   Bucket bandwidth throttling
-   S3 Select on compressed files (GZIP, BZIP2)
-   S3 POST policy (HTML form-based upload)
-   S3 Inventory reports (periodic CSV)
-   Snowball/TAR bulk upload
-   FIFO quota (delete oldest objects when quota exceeded)
-   AMQP/RabbitMQ notification backend (amqp091-go client with lazy connection and topic exchange)
-   PostgreSQL notification backend (lib/pq driver, auto-create table, JSONB storage)
-   Elasticsearch notification backend
-   SSE-KMS encryption (HashiCorp Vault Transit engine + local key provider, key rotation, per-object AES-256-GCM)
