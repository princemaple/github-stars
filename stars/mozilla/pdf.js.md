...
Also build a per-test index (see below).
...
`coverage_search` lists the ref tests that exercised a specific source line or function. It uses the per-test index (`per-test-index.json`) that is rebuilt on every push to `master` and published to the `pdf.js.refs` repository. The index is downloaded on demand, cached locally, and only re-downloaded when it has changed, so no local coverage build is required:
...
To run — or regenerate the reference images for — only the ref tests that touch a given line or function, pass the same `--code` option to a browser test or `makeref` task:
...
Pass `--no-download` to reuse the locally cached index without contacting the network. The index can also be built and queried locally (the CI job that publishes it builds it the same way):
...
On every push and pull request three GitHub Actions workflows collect coverage and upload it to Codecov, each tagged with its own Codecov _flag_ so the test types can be told apart:
