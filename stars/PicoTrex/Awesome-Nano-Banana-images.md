...
-   🍌 Nano Banana Pro 例子
    ...
    -   例 31: 风格化地图（by @Mr\_AllenT）
    -   例 32: 赛博风格名片（by @dotey）
    -   例 33: 精美亚克力板（by @MatoToushi）
    -   例 34: 电脑桌面吉祥物（by @munou\_ac）
    -   例 35: 城市冰箱贴（by @NanoBanana）
    -   例 36: 国家美食地图（by @emollick）
    -   例 37: 城市天气卡片（by @dotey）
    -   例 38: Q 版微缩概念店（by @tetumemo）
    -   例 39: 星穹铁道战斗画面（by @linaqruf\_）
    -   例 40: 根据模切线折叠成盒子（by @Salmaaboukarr）
    -   例 41: 拍立得照片（by @cheese\_ai07）
    -   例 42: 垂直切片失真特效（by @gizakdag）
    -   例 43: 图像UV展开（by @KanaWorks\_AI）
    -   例 44: 鱼眼广角自拍（by @Arminn\_Ai）
    -   例 45: 和电影角色合照（by @rohanpaul\_ai）
    -   例 46: 伪造抖音截图（by @tuzi\_ai）
    -   例 47: 3D 微缩等距房间模型（by @Arminn\_Ai）
    -   例 48: 卫星图像生成等距场景图像（by @langzihan）
    -   例 49: 赛博朋克风格的人物时尚大片（by @MehdiSharifi）
    -   例 50: 四季场景（by @dotey）
    -   例 51: 人物纸艺剪纸（by @neyric\_w）
